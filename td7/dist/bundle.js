(() => {
  ...
  var BASE_IMG = "https://webetu.iutnc.univ-lorraine.fr";

  // src/photoloader.ts
  function loadPicture(idPicture) {
    return fetch(`${API_URL}/photos/${idPicture}`, { credentials: "include" }).then((response) => {
      if (!response.ok) {
  ...
  function loadResource(uri) {
    return fetch(`${BASE_IMG}${uri}`, { credentials: "include" }).then((response) => {
      if (!response.ok) {
        return Promise.reject(new Error(response.statusText));
      }
      return response.json();
  ...
  function displayPicture(photo) {
    ...
    const template = import_handlebars.default.compile(templateEl);
    const urlComplete = BASE_IMG + photo.url.href;
    document.querySelector("#la_photo").innerHTML = template(__spreadValues({ urlComplete }, photo));
  }
  function displayCategorie(categorie) {
    const templateEl = document.querySelector("#categorieTemplate").innerHTML;
    const template = import_handlebars.default.compile(templateEl);
    document.querySelector("#la_categorie").innerHTML = template(__spreadValues({}, categorie));
  }
  ...
  function display_galerie(gal) {
    const res = gal.photos;
    const templateEl = document.querySelector("#galerieTemplate").innerHTML;
    const template = import_handlebars2.default.compile(templateEl);
    const photos = res.map((item) => __spreadProps(__spreadValues({}, item), {
      // on copie toutes les propriétés existantes de la photo
      thumbnailUrl: BASE_IMG + item.photo.thumbnail.href
      // on ajoute l'URL complète
    }));
  ...
  function getPicture(id) {
    loadPicture(id).then((data) => {
      ...
      loadCategorie(data).then((categorie) => displayCategorie(categorie)).catch(handleError);
      loadComments(data).then((comments) => displayComments(comments)).catch(handleError);
    }).catch(handleError);
  }
  ...
  function loadCategorie(data) {
    return loadResource(data.links.categorie.href).then((catData) => catData.categorie);
  }
  function loadComments(data) {
    return loadResource(data.links.comments.href).then((commData) => commData.comments);
  }
  ...
  document.querySelector("#la_galerie").addEventListener("click", (event) => {
    ...
    const vignette = target.closest(".vignette");
    if (!vignette)
      return;
